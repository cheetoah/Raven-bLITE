...
public class HUD extends Module {
   ...
   public HUD() {
      super("Modules", Module.category.hud, 0);

      this.registerSetting(dropShadow = new Tick("Drop shadow", true));
      this.registerSetting(showRender = new Tick("Show render modules", true));
   ...
   private float ssize = (float)Math.pow(1.5,-1);
   ...
   public void a(RenderTickEvent ev) {
      ...
      if (ev.phase == Phase.END && Utils.Player.isPlayerInGame()) {
         ...
         int y = hudY;
         int del = 0;
         this.hudY = ClickGui.getCategoryByName("hud").getY() + 20;
         this.hudX = ClickGui.getCategoryByName("hud").getX();
         
        
         sr = new ScaledResolution(mc);
         HUD.positionMode = Utils.HUD.getPostitionMode(this.hudX, this.hudY,sr.getScaledWidth(), sr.getScaledHeight());
         ...
         if (mResourceLocation != null && logo.isToggled()) {
           // Minecraft.getMinecraft().getTextureManager().bindTexture(mResourceLocation);
            //Gui.drawModalRectWithCustomSizedTexture(hudX, hudY, 0, 0, logoWidth, logoHeight, logoWidth, logoHeight);
        	 GL11.glScaled(1.5D, 1.5D, 1.5D);
        	 mc.fontRendererObj.drawString("Raven b-", (float)(hudX / 1.5), (float)( y / 1.5),  Utils.Client.astolfoColorsDraw(10, 14), false);
        	 GL11.glScaled(ssize, ssize, ssize);
         }
         ...
         if(hudY < 0) {
            if(logo.isToggled() && mResourceLocation != null) {
               hudY = logoHeight - (int)(logoHeight * 0.2);
            } else {
               hudY = margin;
            }
         }
         ...
         while(var5.hasNext()) {
            ...
            if (m.isEnabled() && m != this) {
            	if (showRender.isToggled() || m.moduleCategory() != category.render) {
